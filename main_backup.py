...
# Использует бесплатные ИИ модели через OpenRouter для генерации премиум конта.
...
class AdvancedHealthBot:
    ...
    async def generate_and_publish_content(self):
        ...
        try:
            ...
            if content.get('image_bytes'):
                # Публикуем пост с изображением для лучшей вовлеченности
                ...
                # Изящная деградация: публикуем текстовый пост если изображение не создано
                logger.info(f"📝 Публикация текстового поста (изображение не создано)")
                ...
                self.stats['last_post_date'] = datetime.now()
                    ...
                    self.analytics.record_post_performance({
                        'post_id': post_id,
                        'title': content['title'],
                        'topic': content['topic'],
                        'body': content['body'],
                ...
                logger.info("💰 Монетизация отключена - пропускаем отслеживание дохода")

                    logger.info(f"🎉 Пост опубликован успешно! Всего постов: {self.stats['posts_published']} (частный канал)")
    
                    # Генерируем видео контент (опционально)
                    logger.info("🎥 Генерация видео контента")
                    video_path = self.content_gen.generate_video_content(content)
    
                    if video_path:
                        caption = f"{content['title']}\n\n{content['body'][:200]}...\n\n{' '.join(content.get('hashtags', [])[:5])}"
                        video_success = self.publisher.publish_video_post(video_path, caption)
        ...
        logger.error(f"Ошибка удаления видеофайла: {e}")
            else:
                        logger.info("ℹ️ Видео не создано (опциональная функция)")
        ...
        def generate_analytics_report(self):
            ...
            try:
                ...
                # Генерируем графики
                chart_files = []
                for chart_type in ['engagement', 'topics']:
                chart_file = self.analytics.export_analytics_chart(chart_type)
                    if chart_file:
                        chart_files.append(chart_file)
    
                # Отправляем отчет администратору
            self.publisher._notify_admin("📊 Еженедельный отчет по аналитике:")
            self.publisher._notify_admin(report[:4000])  # Ограничиваем длину сообщения

            # Отправляем графики если есть
            for chart_file in chart_files[:2]:  # Максимум 2 графика
                try:
                    self.publisher._notify_admin(f"📈 График: {chart_type}")
                    # Здесь можно отправить изображение если бот поддерживает
                                except Exception as e:
                    logger.error(f"Ошибка отправки графика аналитики: {e}")
        ...
        try:
            ...
            try:
                ...
                openrouter_ok = bool(test_content)
                                                        ...
                                                        logger.error(f"Ошибка проверки OpenRouter API: {e}e}")
                openrouter_ok = False
    ...
    def startup(self):
        ...
        startup_message = f""
        🤖 Улучшенный Бот Здоровья Запущен!"
